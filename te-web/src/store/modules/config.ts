...
const useConfigStore = defineStore('config', () => {
  ...
  watchPostEffect(() => {
    ...
    document.documentElement.style.setProperty('--trim-sider-width', `${(config.value.theme.siderCollapsed ? 64 : config.value.theme.siderWidth) + (config.value.layout === 'double-columns' ? 80 : 0)}px`,
    )
