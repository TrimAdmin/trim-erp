...
// 生成菜单
export function generateMenu(routes: RouteRecordRaw[], noFilter?: boolean): MenuOption[] | undefined {
  const filteredRoutes = routes.filter((route) => noFilter || !route.meta?.hideMenu)
  return filteredRoutes && filteredRoutes.length
    ? filteredRoutes.reduce((res: MenuOption[], route) => {
      ...
      res.push({
        ...
        icon: menu.meta?.icon
          ...
          : undefined,
        children: menu.children && menu.children.length ? generateMenu(menu.children, noFilter) : undefined,
      })
      return res
    }, [] as MenuOption[])
    : undefined
